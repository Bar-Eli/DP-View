...
export class RuleTable extends Component {
    ...
    RenderFilterChip = (rowData) => {
        if (rowData.filter !== undefined) {
            return (
                <Tooltip
                    title={(rowData.filter.schemaPath != "" ? rowData.filter.schemaPath : rowData.filter.filterType)}
                    aria-label="to">
                    <Chip
                        color="secondary"
    ...
    RenderIcons = (rowData) => {
        return (
            <Tooltip title={rowData.direction === "Source" ? "To DataPower" : "From DataPower"} aria-label="direction">
                    ...
                    actions={[
                        {
                            ...
                            onClick: (event, rowData) => {
                                ...
                                this.setState({
                                    tableData: tableData.filter(element => element.ruleName != rowData.ruleName)
                                });
                                this.props.removeRule(this.props.data.rules.findIndex(rule => rule.name == rowData.ruleName))
                            }
                        },
                    ]}
                />
